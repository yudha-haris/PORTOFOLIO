...
def load_file_peminjam():
	...
	global data_pinjam
	data_pinjam = eval(result)
	overlay_peminjam.destroy()

def save_file_peminjam():
	file_name = asksaveasfilename(filetypes=[("All files", "*")])
	if not file_name:
		return
		
	with open(file_name, "w") as output_file:
		output_file.write(str(data_pinjam))
		overlay_peminjam.destroy()  

def load_file_katalog():
	file_name = askopenfilename(filetypes=[("All files", "*")])
	if not file_name:
		return
	text_file = open(file_name, 'r', encoding="utf-8")
...
def status():
	...
	frame_TOP = Frame(overlay_status)
	frame_TOP.pack(side=TOP)
	Button(frame_TOP, text="Katalog", relief=FLAT).pack(side= LEFT, fill=BOTH, expand=YES)
	Button(frame_TOP, text="Peminjam", command=pinjaman).pack(side=LEFT, fill=BOTH, expand=YES)
	Label(overlay_status, text="\nSTATUS", font=("Metropolis Black", 15)).pack()
	Button(frame_BOT, text="SAVE", command=save_file_katalog).pack(side=LEFT, pady=10)
	Label(frame_BOT, text="	").pack(side=LEFT)
	Button(frame_BOT, text="LOAD", command=load_file_katalog).pack(side=LEFT, pady=10)
	...
	global result_katalog
	result_katalog = scrolledtext.ScrolledText(overlay_status,  width = 40,  height = 10, font=("Times New Roman",10))
	result_katalog.pack()
	result_katalog.insert(INSERT, result_status_data)
	result_katalog.configure(state=DISABLED)
	
def pinjaman():
	# GUI
	overlay_status.destroy()
	global overlay_peminjam
	overlay_peminjam = Toplevel()
	overlay_peminjam.geometry("300x300")
	frame_BOT = Frame(overlay_peminjam)
	frame_BOT.pack(side=BOTTOM)
	frame_TOP = Frame(overlay_peminjam)
	frame_TOP.pack(side=TOP)
	Button(frame_TOP, text="Katalog", command=oper).pack(side= LEFT, fill=BOTH, expand=YES)
	Button(frame_TOP, text="Peminjam", relief=FLAT).pack(side=LEFT, fill=BOTH, expand=YES)
	Label(overlay_peminjam, text="\nSTATUS", font=("Metropolis Black", 15)).pack()
	Button(frame_BOT, text="SAVE", command=save_file_peminjam).pack(side=LEFT, pady=10)
	Label(frame_BOT, text="	").pack(side=LEFT)
	Button(frame_BOT, text="LOAD", command=load_file_peminjam).pack(side=LEFT, pady=10)
	...
	if status_data == []:
		...
		result_status_data = "\n".join(status_data)
	result_peminjam = scrolledtext.ScrolledText(overlay_peminjam,  width = 40,  height = 10, font=("Times New Roman",10))
	result_peminjam.pack()
	result_peminjam.insert(INSERT, result_status_data)
	result_peminjam.configure(state=DISABLED)
...
class Underground(Tk):
	...
	def __init__(self):
		...
		self.frames["return_buku"] = return_buku(master=container, controller=self)
		...
		self.frames["return_buku"].grid(row=0, column=0, sticky="NSEW")
...
class buku_menu(Frame):
	...
	def make_widget(self):
		...
		Button(self, text="   TAMBAHKAN BUKU   ", command=lambda: self.controller.show_frame("add_buku"), relief=SOLID, font=("Metropolis", 12, "bold")).grid(row=2,
		 column=0, padx= 10, pady=10, rowspan=2)
		...
		Button(self, text="  TAMBAH STOK BUKU  ", command=lambda: self.controller.show_frame("add_stok_buku"), relief=SOLID, font=("Metropolis", 12, "bold")).grid(row=6,
		 column=0, padx= 10, pady=10, rowspan=2)
		Button(self, text="PENGEMBALIAN BUKU", command=lambda: self.controller.show_frame("return_buku"), relief=SOLID, font=("Metropolis", 12, "bold")).grid(row=8,
		 column=0, padx= 10, pady=10, rowspan=2)
...
class sub_buku(Frame):
	...
	def make_widget(self):
		...
		self.e0 = Entry(self, width="30", textvariable= self.peminjam).grid(row=2, column=0,sticky = E, columnspan=2)
				...
				if self.nama_buku.get() == y[i]:
					if int(y[i+5]) < int(self.jumlah.get()):
						...
						if self.peminjam.get() in data_pinjam.keys():
							...
							data_pinjam.update({self.peminjam.get():[self.nama_buku.get(),self.jumlah.get()]})
						config = 1
						break
		if config == 0:
			Label(overlay, text=f"Buku dengan nama {self.nama_buku.get()} tidak ada!").pack()

class return_buku(Frame):
	# Constructor untuk class add_rak()
	def __init__(self, master, controller):
		self.controller = controller
		self.master = master
	...
	def make_widget(self):
		...
		# LABEL
		Label(self, text="Pengembalian", font=("Metropolis Black", 20)).grid(row=1,column=0, columnspan=2, padx=60, pady=20)
		Label(self, text="PEMINJAM	").grid(row=2, sticky=W, padx= 20)
		Label(self, text="NAMA BUKU	").grid(row=3, sticky=W, padx= 20)
		Label(self, text="JUMLAH	").grid(row=4, sticky=W, padx= 20)	
		...
		# BUTTON
		Button(self, text="<< BACK", command=lambda: self.controller.show_frame("buku_menu"), relief=FLAT, font=("Metropolis", 10), bg="#003B7A", fg="white").grid(row=8, column=0, sticky=W, padx= 20, pady=50)
		Button(self, text="KEMBALIKAN", command=self.kembalian_buku, relief=FLAT, font=("Metropolis", 10), bg="#003B7A", fg="white").grid(row=8, column=1, sticky=E, padx= 20, pady=50)

	def kembalian_buku(self):
		# GUI
		overlay = Toplevel(self)
		overlay.geometry("300x50")
			...
			if self.nama_buku.get() in data_pinjam[self.peminjam.get()]:
				...
				for x,y in database.items():
					for i in range(len(y)):
						if self.nama_buku.get() == y[i]:
							y[i+5] = str(int(y[i+5]) + int(self.jumlah.get()))
							Label(overlay, text=f"Buku dengan nama {self.nama_buku.get()}\nsudah dikembalikan oleh {self.peminjam.get()}").pack()
							print(data_pinjam)
							break
			else:
				Label(overlay, text=f"Buku dengan nama {self.nama_buku.get()}\n tidak ada atau tidak dipinjam!").pack()
		else:
			Label(overlay, text=f"Peminjam atas nama {self.peminjam.get()} tidak ada!").pack()
...
class add_stok_buku(Frame):
	...
	def make_widget(self):
		...
		# BUTTON
		Button(self, text="<< BACK", command=lambda: self.controller.show_frame("buku_menu"), relief=FLAT, font=("Metropolis", 10), bg="#003B7A", fg="white").grid(row=8, column=0, sticky=W, padx= 20, pady=50)
		Button(self, text="TAMBAH", command=self.tambah_stok, relief=FLAT, font=("Metropolis", 10), bg="#003B7A", fg="white").grid(row=8, column=1, sticky=E, padx= 20, pady=50)
